...
/**
 * Computes the next-run preview for a schedule input (design doc §21.2).
 * Returns the next instant plus local + UTC display strings. Pure — does not
 * touch the database, so it works even if the scheduler hasn't started.
 */
export async function POST(req: Request) {
  try {
    ...
    if ((s.type !== "daily" && s.type !== "once") || typeof s.timezone !== "string") {
      return NextResponse.json(
        { error: "schedule.type and schedule.timezone are required", code: "VALIDATION_ERROR" },
        { status: 400 },
      );
    }
    const input =
