...
/*
 * Key-shaped fixtures are assembled at runtime instead of written as literals.
 *
 * Redaction can only be tested against the real prefixes, but a literal
 * "AIzaSy…" or "ghp_…" sitting in a committed file trips GitHub secret
 * scanning — and an alert stream full of known-fake test data is an alert
 * stream nobody reads, which is how a real leak gets missed. Concatenation
 * keeps the string handed to redactSecrets() byte-identical while leaving no
 * scannable literal in the source.
 *
 * None of these were ever real credentials; they are sequential alphabets.
 */
...
test("redacts provider key shapes before text is stored", () => {
  ...
  assert.match(redactSecrets(`use ${shaped("sk-", "or-", "v1-", LOWER, "123456")}`), /\[redacted\]/);
  assert.match(redactSecrets(`token ${shaped("ghp", "_", LOWER, "12")}`), /\[redacted\]/);
  assert.match(redactSecrets(shaped("tskey", "-auth-", LOWER.slice(0, 16))), /\[redacted\]/);
  assert.match(redactSecrets(shaped("AIza", "Sy", UPPER, "0123456")), /\[redacted\]/);
  assert.match(redactSecrets(shaped("api", "_key: \"", "sup3rsecretvalue123", "\"")), /\[redacted\]/);
  // The label survives so the sentence still reads, only the value is gone.
  assert.match(
    redactSecrets(shaped("api", "_key: ", "sup3rsecretvalue123")),
    /^api_key: \[redacted\]$/,
  );
});
