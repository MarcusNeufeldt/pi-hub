...
test("cleans common session title response wrappers", () => {
  ...
  assert.equal(parseGeneratedSessionTitle("BOL - 3528 Customer default project"), "BOL - 3528 Customer default project");
  assert.equal(Array.from(parseGeneratedSessionTitle("A".repeat(95))).length, 90);
});
...
test("folds the title request into a trailing user message without mutating the source", () => {
  ...
  const titlePrompt = prepared[1].content.at(-1).text;
  assert.match(titlePrompt, /Create a concise sidebar title/);
  assert.match(titlePrompt, /first five words must identify what the whole session was about/i);
  assert.match(titlePrompt, /dominant end-to-end arc/i);
  assert.match(titlePrompt, /BOL - 1234/);
  assert.match(titlePrompt, /PR - 1234/);
  assert.match(titlePrompt, /3-9 words, no more than 90 characters, and ASCII/i);
...
test("removes incomplete tool calls before invoking the title provider", async () => {
  ...
  assert.match(providerMessages[0].content, /Create a concise sidebar title/);
});
