...
test("queued and running count as live; paused and terminal states do not", async () => {
  // Matches pi-subagents' own isActiveAsyncState (queued || running). An
  // interrupted run is finalized as "paused", so it must not hold a session.
  const states = {
    queued: true,
    running: true,
    paused: false,
...
test("a live runner pid keeps the session even when status.json looks ancient", async () => {
  // lastUpdate is not a heartbeat: the runner only rewrites status.json when the
  // activity classification changes, so a live run in one long quiet tool call
  // leaves a stale file. The pid is what stays true.
  const dir = await runsDir({
    quiet: { runId: "quiet", sessionId: "S", state: "running", lastUpdate: NOW - DEFAULT_STALE_RUN_MS * 10, pid: 4242 },
  });
  try {
    assert.equal(hasLiveSubagentWork("S", { dir, now: NOW, isAlive: () => true }), true, "alive pid wins over file age");
    assert.equal(hasLiveSubagentWork("S", { dir, now: NOW, isAlive: () => false }), false, "dead pid ends it immediately");
  } finally {
    await rm(dir, { recursive: true, force: true });
  }
});

test("a dead runner pid is not kept alive by a fresh status file", async () => {
  const dir = await runsDir({ dead: { runId: "dead", sessionId: "S", state: "running", lastUpdate: NOW, pid: 4242 } });
  try {
    assert.equal(hasLiveSubagentWork("S", { dir, now: NOW, isAlive: () => false }), false);
  } finally {
    await rm(dir, { recursive: true, force: true });
  }
});

test("falls back to file age only when no pid is recorded", async () => {
  const fresh = await runsDir({ r: { runId: "r", sessionId: "S", state: "running", lastUpdate: NOW } });
  const old = await runsDir({ r: { runId: "r", sessionId: "S", state: "running", lastUpdate: NOW - DEFAULT_STALE_RUN_MS - 1 } });
  try {
    assert.equal(hasLiveSubagentWork("S", { dir: fresh, now: NOW }), true);
    assert.equal(hasLiveSubagentWork("S", { dir: old, now: NOW }), false);
  } finally {
