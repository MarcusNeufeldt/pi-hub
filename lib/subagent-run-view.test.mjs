...
test("omits only a final assistant narration that duplicates the Result", async () => {
  const dir = await mkdtemp(join(tmpdir(), "pi-hub-subagent-duplicate-result-"));
  const transcriptPath = join(dir, "duplicate_transcript.jsonl");
  ...
  try {
    ...
    const rows = [
      {
        ...
        role: "assistant",
        timestamp: "2026-08-09T05:00:01.000Z",
        text: "I am checking the final test output now.",
      },
      {
        recordType: "message",
        sourceEventType: "message_end",
        ...
        role: "assistant",
        timestamp: "2026-08-09T05:00:02.000Z",
        text: `${result}\n\nAcceptance metadata that is not part of the rendered Result.`,
      },
    ];
    ...
    assert.equal(view.children[0].finalOutput, result);
    assert.deepEqual(
      view.children[0].events.filter((event) => event.kind === "assistant").map((event) => event.detail),
      ["I am checking the final test output now."],
    );
  } finally {
    await rm(dir, { recursive: true, force: true });
  }
...
test("pairs results with steps by position when a fanout repeats one agent", async () => {
  // `{ agent: "scout", tasks: [a, b, c] }` names every child "scout". Matching a
  // result to a step by agent name gave all three the FIRST step, so every child
  // reported the same transcript path and shared one timeline cursor.
  const status = {
    state: "running",
    steps: [
      { agent: "scout", transcriptPath: "/runs/scout-0.jsonl" },
      { agent: "scout", transcriptPath: "/runs/scout-1.jsonl" },
      { agent: "scout", transcriptPath: "/runs/scout-2.jsonl" },
    ],
    workflow: {
      value: {
        results: [
  ...
  const view = await buildSubagentRunView(status, []);

  assert.equal(view.children.length, 3);
  assert.deepEqual(view.children.map((c) => c.index), [0, 1, 2]);
  assert.deepEqual(view.children.map((c) => c.task), ["first", "second", "third"]);
  assert.deepEqual(
    view.children.map((c) => c.transcriptPath),
    ["/runs/scout-0.jsonl", "/runs/scout-1.jsonl", "/runs/scout-2.jsonl"],
    "each child keeps its own transcript rather than the first child's",
  );
});

test("still matches a step by agent name when the counts differ", async () => {
  const status = {
    state: "running",
    steps: [{ agent: "worker", transcriptPath: "/runs/worker.jsonl" }],
    workflow: { value: { results: [{ agent: "worker", task: "only" }] } },
  };
  const view = await buildSubagentRunView(status, []);
  assert.equal(view.children.length, 1);
  assert.equal(view.children[0].agent, "worker");
  assert.equal(view.children[0].transcriptPath, "/runs/worker.jsonl");
});
